...
#  Примеры:
#    restore.sh avito_zamer -y
#    restore.sh ebay /root/postgres-docker/db_backups/ebay/2025-08-10/ebay_2025-08-10_13-26.sql.gz -y
#    echo /root/postgres-docker/db_backups/test_db/2025-08-10/test_db_*.sql.gz > /root/postgres-docker/db_backups/test_db/SELECTED
#    restore.sh test_db -y
# ------------------------------------------------------------
...
for ENV_FILE in "$ROOT_DIR"/*/.env; do
  [[ -f "$ENV_FILE" ]] || continue
  local_svc="$(basename "$(dirname "$ENV_FILE")")"
...
if [[ -n "${FILE_ARG:-}" ]]; then
  ...
  if [[ -z "${BACKUP_FILE:-}" || ! -f "$BACKUP_FILE" ]]; then
    BACKUP_FILE="$(find "$BACKUP_DIR/$SVC" -type f -name "${DBNAME}_*.sql.gz" -printf '%T@ %p\n' 2>/dev/null \
                  | sort -nr | head -n1 | cut -d' ' -f2- || true)"
  fi
fi

[[ -n "${BACKUP_FILE:-}" && -f "$BACKUP_FILE" ]] || { echo "Бэкап не найден для '$SVC' (ожидал ${BACKUP_DIR}/${SVC}/${DBNAME}_*.sql.gz или файл в SELECTED)."; exit 1; }
